...
export const Web3Provider = ({ children }: Props) => {
  const [h160Account, setH160Account] = useState("");
  ...
  const initEvmProvider = useCallback(
    async () => {
      if (!window.ethereum) {
  ...
  return (
    <WalletContext.Provider
      value={{ wallet, isConnectedToEvm, setWallet, initEvmProvider, getAccounts }}
    >
      {children}
    </WalletContext.Provider>
  );
